...
func (r *fileSystemModuleRegistry) QueryAllModuleHeaders() ([]header.ModuleHeader, error) {
	var headers []header.ModuleHeader
	...
	for _, file := range files {
		if file.IsDir() {
			...
			headers = append(headers, &header.FileSystemModuleHeader{
				Name:     file.Name(),
...
func (r *fileSystemModuleRegistry) QueryModuleHeaders(lbls []label.ModelFrameResourceLabel) ([]header.ModuleHeader, error) {
	var headers []header.ModuleHeader
	f, err := os.Open(r.ModulesDir)
	if err != nil {
		return headers, errors.WithStack(err)
